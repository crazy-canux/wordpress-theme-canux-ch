...
<div class="right_mian"<?php if($left_right){echo 'style="float: left;"';}  ?>>
 ...
 <ul class="default">
   ...
   <?php
   if(get_option('mytheme_list_nmber2')==""){$nmnber =12;}else{ $nmnber =get_option('mytheme_list_nmber2');}
$posts = query_posts($query_string . '&showposts='.$nmnber); ?>
        ...
        <?php
		 ...
		 ?>
               <li>
                  <a <?php echo $tagerts ; ?> href="<?php echo $links1 ?>" class="picdsa">
                  <?php  if( is_sticky()&&!$stickys){echo '<div id="tuijian_loop" class="tuijian_loop"></div>';} ?>
                   <?php  canuxtheme_thumbnails('news'); ?> </a>
                  <span>
