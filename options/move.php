...
function mytheme_move_opion($wp_customize){
	  ...
	  $wp_customize->add_section('mytheme_detects_scheme', array(
       ...
       'description' => 'useless </br>  <a href="http://www.canuxcheng.com" target="_blank">useless</a>  </br>',
        'priority' => 79,
    ));
    ...
    $wp_customize->add_control('mytheme_move_tel_name', array(
		...
		'description' => '此处以及以下选项为手机版底部图标名称，你可以替换掉他们，以两个字为最佳，若需要不显示文字而显示纯图标，可以填写空格.',
		    ...
		    $wp_customize->add_setting('mytheme_move_tell', array(
        'default'        => '+86-13681984515',
        'capability'     => 'edit_theme_options',
        'type'           => 'option',

    ));

    $wp_customize->add_control('mytheme_move_tell', array(
        'label'      => __('电话号码', 'mytheme_move_tell'),
        'section'    => 'mytheme_detects_scheme',
        'settings'   => 'mytheme_move_tell',
		'description' => '请填写电话号码',
    ));
    ...
    $wp_customize->add_control('mytheme_QQ', array(
        ...
        'section'    => 'mytheme_detects_scheme',
        'settings'   => 'mytheme_QQ',
		'description' => '进入qq互联或者其他skype等软件，获取代码，填写herf=里面的url',
    ));
